<?php
    ...
    if ($favorite_post_ids) {
      ...
      if (!isset($j)) {
        ...
        if($i ==1 && get_field('destacado') == TRUE) echo '</div>';
        if($j == 0 || get_field('destacado') == TRUE) echo '<div class="box">';
        $i = 1;
    ?>
    ...
    <?php
        ...
        if($j == 4  || get_field('destacado') == TRUE) {
        $i = 0;
          echo '</div>';
          $j = 0;
        }

        if($wp_query->current_post == $wp_query->post_count && $j == 4  && get_field('destacado') == TRUE) echo '</div>';
