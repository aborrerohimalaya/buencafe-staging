...
function wpfp_link_html($post_id, $opt, $action) {
    ...
    $link = "<a class='wpfp-link pin ".$action."' href='?wpfpaction=".$action."&amp;postid=". $post_id . "' title='". $opt ."' rel='nofollow'></a>";
    $link = apply_filters( 'wpfp_link_html', $link );
    return $link;
}

function getImgFavoritePostType($user = "", $postType) {
    ...
    $favoritos = array_reverse($favorite_post_ids);

    foreach ($favoritos as $favorito) {
        if (get_post_type($favorito) == $postType) {
            //array_push($favoritos, $favorito);
            break;
        }
    }

    $image = wp_get_attachment_image_src( get_post_thumbnail_id( $favorito ), 'full' );
...
function wpfp_remove_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        ...
        $link = "<a id='rem_$post_id' class='$class pin remover' href='?wpfpaction=remove&amp;page=1&amp;postid=". $post_id ."' title='".wpfp_get_option('rem')."' rel='nofollow'>".wpfp_get_option('rem')."</a>";
...
function wpfp_clear_list_link() {
    if (wpfp_is_user_can_edit()) {
        ...
        echo "<a class='wpfp-link pin remover' href='?wpfpaction=clear' rel='nofollow'>". wpfp_get_option('clear') . "</a>";
    }
}
