...
//            }
    ...
    */
    ...
    function make_duplicate($master_post_id, $lang){
        ...
        $query_mappress_post  = 'SELECT * FROM wp_mappress_posts WHERE postid = "'.$master_post_id.'"';
        $result_mappress_post = $wpdb->get_results($query_mappress_post);
    
         if(count($result_mappress_post)>0){
             $wpdb->query("UPDATE  wp_log SET text='" .  json_encode($result_mappress_post[0]->mapid)."'  WHERE id = '1'");
             $wpdb->insert( 'wp_mappress_posts', array('postid'=>$id,'mapid'=>$result_mappress_post[0]->mapid) ,array( '%d','%d' ));
         }

        
       // var_dump($master_post_id, $lang, $postarr, $id);exit;
        return $ret;
