...
function get_post_reply_link($args = array(), $post = null) {
	$defaults = array(
		'add_below'  => 'post',
		'respond_id' => 'respond',
		'reply_text' => __('Leave a Comsment'),
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<div class="usuario"><img alt="usuario" src="'.get_field('foto_usuario', 'user_'.$user->ID).'"></div><textarea id="comment" name="comment" cols="45" rows="8" aria-describedby="form-allowed-tags" aria-required="true" required="required"></textarea>',
		/** This filter is documented in wp-includes/link-template.php */
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'Disculpá, debés <a href="%s">iniciar sesión</a> para escribir un comentario.' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		/** This filter is documented in wp-includes/link-template.php */
		//'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		//'logged_in_as'         => '<div class="usuario"><img alt="usuario" src="'.get_field('foto_usuario', 'user_'.$user->ID).'"></div>',
		'logged_in_as'         => '',
		...
		'submit_button'        => '    <span class="submit">Enviar</span><input name="%1$s" type="submit" id="%2$s" class="submit" value="%4$s" />',
